...
#SBATCH --reservation=benchmarks                  # Queue. Use 'batch' for most jobs, or 'scavenger' for quick tests.
#SBATCH --qos=benchmark
#SBATCH --ntasks=32                    # Total number of MPI processes in job.
#SBATCH --nodes=16                     # Number of nodes in job.
#SBATCH --ntasks-per-node 2           # Number of MPI processes per node.
#SBATCH --cpus-per-task 20            # Number of OMP threads spawned from each MPI process.
...
omp_threads_per_mpi_rank=20           # Repeat the value from 'cpus-per-task' here.
